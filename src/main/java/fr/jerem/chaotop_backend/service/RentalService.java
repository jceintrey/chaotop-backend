 ...
 * This method is intended for use in controllers to return a DTO representation of the rental.
 * 
 * @param id the unique identifier of the rental.
 * @return an {@link Optional} containing the {@link RentalResponse} if found, otherwise empty.
 */
    Optional<RentalResponse> getRentalById(Long id);

/**
 * Retrieves a rental entity by its unique identifier.
 * <p><b>Note:</b> This method is intended for internal service use only and should not be used in controllers.
 * Controllers should use {@link #getRentalById(Long)} to ensure DTO encapsulation.</p>
 * 
 * @param id the unique identifier of the rental.
 * @return an {@link Optional} containing the {@link RentalEntity} if found, otherwise empty.
 */
